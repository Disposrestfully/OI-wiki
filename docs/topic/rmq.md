...
## 较为简单的$O(n)-O(1)$RMQ实现方法

运用和上文中一样的序列分块做法，把连续的$\log n$个数分成一块，取出每一块的最值建$ST$表，复杂度$O(n)$。

现在考虑询问区间的两个端点落在同一块内如何处理。

在预处理的时候，从左到右遍历每个块，同时维护一个单调栈。那么对于询问$[l,r]$，只需要找到时刻$r$所对应的单调栈中第一个不小于$l$的下标。由于每一块只有$\log n$个数，我们可以把单调栈内数字的下标信息压缩成一个二进制数，查询时可以用$builtin$函数做到$O(1)$。
